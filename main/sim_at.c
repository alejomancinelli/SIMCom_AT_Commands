...
static char* sim_at_err_to_str(sim_at_err_t err) {
    ESP_LOGI(TAG, "%d", err);
    switch(err)
    {
...
static sim_at_err_t prv_uart_write_cmd(const char *cmd) {
    ...
    int len = strlen(cmd);
    // uart_flush(g_cfg.uart_port); // TODO: Por las dudas
    int written = uart_write_bytes(g_cfg.uart_port, cmd, len);
    if (written != len) return SIM_AT_ERR_UART;
    if (g_debug) ESP_LOGI(TAG, "--> %s", cmd); // TODO: Limpiar el /r/n del final de ser posible
    return SIM_AT_OK;
}
    ...
    while (1) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c == '\n') {
                ...
                if (g_debug) ESP_LOGI(TAG, "<-- %s", s_line_buf);
        ...
        //     if (g_debug) ESP_LOGI(TAG, "<-- %s", data); 
        //     // TODO: Ver esto de buffer, 
        //     //porque capaz conviene un buffer chiquito e ir incrementando hasta que aparezca \n

        //     // // copy to response buffer
        //     // strncpy(s_response_buf, (char*)data, sizeof(s_response_buf)-1);
        //     // s_response_buf[sizeof(s_response_buf)-1] = '\0';
        //     // s_response_len = len;
...
sim_at_err_t sim_at_init(const sim_at_config_t *cfg) {
    if (!cfg) return SIM_AT_ERR_INVALID_ARG;
    if (g_inited) return SIM_AT_OK; /* idempotent */
    ...
    if (!s_sync_sem) return SIM_AT_ERR_NO_MEM;
...
sim_at_err_t sim_at_cmd_sync(const char *cmd, char *resp_buf, size_t resp_buf_len, uint32_t timeout_ms) {
    if (!g_inited) return SIM_AT_ERR_NOT_INIT;
    if (!cmd || !resp_buf || resp_buf_len == 0) return SIM_AT_ERR_INVALID_ARG;
    if (strlen(cmd) >= SIM_AT_MAX_CMD_LEN) return SIM_AT_ERR_INVALID_ARG;
    ...
    /* completion occurred; copy response to user buffer */
    return SIM_AT_OK;
}
...
sim_at_err_t sim_at_control_dtr(bool state) {
    if (!g_inited) return SIM_AT_ERR_NOT_INIT;
    if (g_cfg.control_pins.dtr_pin < 0) return SIM_AT_ERR_INVALID_ARG;
...
sim_at_err_t sim_at_control_pwrkey(bool state) {
    if (!g_inited) return SIM_AT_ERR_NOT_INIT;
    if (g_cfg.control_pins.pwrkey_pin < 0) return SIM_AT_ERR_INVALID_ARG;
...
sim_at_err_t sim_at_control_reset(bool state) {
    if (!g_inited) return SIM_AT_ERR_NOT_INIT;
    if (g_cfg.control_pins.rst_pin < 0) return SIM_AT_ERR_INVALID_ARG;
